...
import { createNewProject, DEFAULT_COMBINED_FOOTING_INPUTS, DEFAULT_STRAP_FOOTING_INPUTS, DEFAULT_STRIP_FOOTING_INPUTS, isProjectDocument, normalizeProjectDocument, type CombinedFootingInputs, type FootingInputs, type ProjectDocument, type StrapFootingInputs, type StripFootingInputs } from './domain/projects'
...
import { CombinedCaseDefinition } from './ui/combined-footing/CombinedCaseDefinition'
...
import { CombinedCalculationMemo } from './ui/combined-footing/CombinedCalculationMemo'
import { StrapFootingLongitudinalSheet, StrapFootingPlanSheet, StrapReinforcementSummary } from './components/StrapFootingTechnicalSheets'
import { StrapCaseDefinition } from './ui/strap-footing/StrapCaseDefinition'
...
import { StrapCalculationMemo } from './ui/strap-footing/StrapCalculationMemo'
...
const combinedExample: CombinedFootingInputs = { ...DEFAULT_COMBINED_FOOTING_INPUTS }
const strapExample: StrapFootingInputs = { ...DEFAULT_STRAP_FOOTING_INPUTS }
const decimal = /^(?:\d+(?:[.,]\d*)?|[.,]\d*)?$/
...
export default function NewApp() {
  ...
  const clear = () => { setAnalysis(null); setStripAnalysis(null); setCombinedAnalysis(null); setStrapAnalysis(null) }
  ...
  const changeFootingType = (footingType: ProjectDocument['footingType']) => { clear(); setProject((current) => ({ ...current, footingType })); setView('define'); setOpenGroup('loads'); setStatus(footingType === 'strip' ? 'Zapata corrida seleccionada. Las cargas se expresan por metro lineal.' : footingType === 'combined' ? 'Zapata combinada seleccionada. Declara cargas y posición de las dos columnas.' : footingType === 'strap' ? 'Zapata medianera seleccionada. La viga centradora enlaza dos bases sin apoyar en el suelo.' : 'Zapata aislada seleccionada. Las cargas corresponden a la columna centrada.') }
  const loadExample = () => { if (isStrip) { setProject((current) => ({ ...current, name: 'Zapata corrida · caso didáctico', stripInputSnapshot: stripExample })); setStripText(stripInputTextFrom(stripExample)) } else if (isCombined) { setProject((current) => ({ ...current, name: 'Zapata combinada · caso didáctico', combinedInputSnapshot: combinedExample })); setCombinedText(combinedInputTextFrom(combinedExample)) } else if (isStrap) { setProject((current) => ({ ...current, name: 'Zapata medianera · caso didáctico', strapInputSnapshot: strapExample })); setStrapText(strapInputTextFrom(strapExample)) } else { setProject((current) => ({ ...current, name: 'Zapata rectangular · caso didáctico', inputSnapshot: example })); setText(inputTextFrom(example)) }; clear(); setOpenGroup('loads'); setStatus('Ejemplo didáctico cargado. No representa un diseño aprobado.') }
  const analyze = () => {
    try {
      if (isStrap) {
  ...
  const newProject = () => { const fresh = createNewProject(); setProject(fresh); setText(inputTextFrom(fresh.inputSnapshot)); setStripText(stripInputTextFrom(fresh.stripInputSnapshot)); setCombinedText(combinedInputTextFrom(fresh.combinedInputSnapshot)); setStrapText(strapInputTextFrom(fresh.strapInputSnapshot)); clear(); setMenuOpen(false); setView('define'); setStatus('Proyecto nuevo. Completa los datos o carga un ejemplo didáctico.') }
  ...
  return <main className="new-app"><header className="new-topbar"><a href="/" className="new-brand"><b>A</b><span>AndeLogic <strong>Zapatas</strong></span></a><nav aria-label="Vistas de cálculo">{workspaceNavigation.map(({ id, label }) => <button className={view === id ? 'active' : ''} onClick={() => setView(id)} key={id}>{label}</button>)}</nav><span className="technical-profile">{report.profile.shortLabel}</span><div className="file-menu"><button onClick={() => setMenuOpen((open) => !open)} aria-expanded={menuOpen}>Proyecto ⌄</button>{menuOpen && <div><button onClick={newProject}>Nuevo proyecto</button><button onClick={() => void save()}>Guardar</button><button onClick={exportFile}>Descargar archivo</button><button onClick={() => importRef.current?.click()}>Importar archivo</button><button onClick={() => window.print()}>Imprimir informe</button>{projects.map((item) => <button className="stored" onClick={() => void (async () => { const found = await browserProjectRepository.get(item.projectId); if (found) { setProject(found); setText(inputTextFrom(found.inputSnapshot)); setStripText(stripInputTextFrom(found.stripInputSnapshot)); setCombinedText(combinedInputTextFrom(found.combinedInputSnapshot)); setStrapText(strapInputTextFrom(found.strapInputSnapshot)); clear(); setView('define'); setMenuOpen(false); setStatus('Proyecto abierto. Vuelve a analizar el caso.') } })()} key={item.projectId}>{item.name}</button>)}</div>}</div></header><section className="new-project-strip"><label><span>PROYECTO</span><input value={project.name} onChange={(event) => setProject((current) => ({ ...current, name: event.target.value }))}/></label><div className="footing-type-selector" aria-label="Tipo de zapata"><button className={isIsolated ? 'active' : ''} onClick={() => changeFootingType('isolated')}>Aislada</button><button className={isStrip ? 'active' : ''} onClick={() => changeFootingType('strip')}>Corrida</button><button className={isCombined ? 'active' : ''} onClick={() => changeFootingType('combined')}>Combinada</button><button className={isStrap ? 'active' : ''} onClick={() => changeFootingType('strap')}>Medianera</button></div><p>{isStrip ? 'Zapata corrida · muro y carga lineal centrados · franja de 1.00 m.' : isCombined ? 'Zapata combinada rectangular · dos columnas interiores alineadas · presión lineal.' : isStrap ? 'Zapata medianera · dos bases separadas · viga centradora sin apoyo en el suelo.' : 'Zapata aislada rectangular · columna y carga axial centradas.'}</p><button className="secondary-action" onClick={loadExample}>Cargar ejemplo didáctico</button><button className="main-action" onClick={analyze}>Analizar {isStrip ? 'corrida' : isCombined ? 'combinada' : isStrap ? 'medianera' : 'zapata'} →</button></section>
    ...
    {view === 'define' && isCombined && <CombinedCaseDefinition project={project} text={combinedText} openGroup={openGroup} formatIssue={formatIssue} analysis={combinedAnalysis} onOpenGroup={setOpenGroup} onNameChange={(name) => setProject((current) => ({ ...current, name }))} onFieldChange={changeCombined} onBasisChange={(bearingCapacityBasis) => { clear(); setProject((current) => ({ ...current, combinedInputSnapshot: { ...current.combinedInputSnapshot, bearingCapacityBasis } })); setStatus('Base actualizada: vuelve a analizar la zapata combinada.') }} onAnalyze={analyze}/>}
    {view === 'define' && isStrap && <StrapCaseDefinition project={project} text={strapText} openGroup={openGroup} formatIssue={formatIssue} analysis={strapAnalysis} onOpenGroup={setOpenGroup} onNameChange={(name) => setProject((current) => ({ ...current, name }))} onFieldChange={changeStrap} onBasisChange={(bearingCapacityBasis) => { clear(); setProject((current) => ({ ...current, strapInputSnapshot: { ...current.strapInputSnapshot, bearingCapacityBasis } })); setStatus('Base actualizada: vuelve a analizar la zapata medianera.') }} onAnalyze={analyze}/>}
    ...
    {view === 'section' && isStrap && <ViewShell number="02" title="Sección longitudinal del sistema" detail="Excentricidad, transferencia por la viga y reacciones independientes bajo las dos bases."><StrapFootingLongitudinalSheet inputs={project.strapInputSnapshot} analysis={strapAnalysis}/></ViewShell>}
    ...
    {view === 'plan' && isStrap && <ViewShell number="03" title="Planta y armado del sistema" detail="Lindero, dos bases, viga centradora y resumen explícito del armado preliminar."><div className="new-plan-grid"><StrapFootingPlanSheet inputs={project.strapInputSnapshot} analysis={strapAnalysis}/><StrapReinforcementSummary inputs={project.strapInputSnapshot} analysis={strapAnalysis}/></div></ViewShell>}
