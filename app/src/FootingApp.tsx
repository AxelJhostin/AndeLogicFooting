...
import { createNewProject, DEFAULT_COMBINED_FOOTING_INPUTS, DEFAULT_STRIP_FOOTING_INPUTS, isProjectDocument, normalizeProjectDocument, type CombinedFootingInputs, type FootingInputs, type ProjectDocument, type StripFootingInputs } from './domain/projects'
...
export default function NewApp() {
  ...
  const clear = () => { setAnalysis(null); setStripAnalysis(null); setCombinedAnalysis(null) }
  ...
  const newProject = () => { const fresh = createNewProject(); setProject(fresh); setText(inputTextFrom(fresh.inputSnapshot)); setStripText(stripInputTextFrom(fresh.stripInputSnapshot)); setCombinedText(combinedInputTextFrom(fresh.combinedInputSnapshot)); clear(); setMenuOpen(false); setView('define'); setStatus('Proyecto nuevo. Completa los datos o carga un ejemplo didáctico.') }
