...
import { createNewProject, isProjectDocument, normalizeProjectDocument, type FootingInputs, type ProjectDocument } from './domain/projects'
import { applyFootingExample, defaultExampleForFootingType, examplesForFootingType } from './domain/examples/footing-examples'
...
export default function NewApp() {
  ...
  const [selectedExampleId, setSelectedExampleId] = useState(() => defaultExampleForFootingType(project.footingType).id)
  ...
  const clear = () => { setAnalysis(null); setStripAnalysis(null); setCombinedAnalysis(null); setStrapAnalysis(null); setTrapezoidalAnalysis(null); setEdgeAnalysis(null) }
  ...
  const activeExamples = examplesForFootingType(project.footingType)
  const selectedExample = activeExamples.find(({ id }) => id === selectedExampleId) ?? defaultExampleForFootingType(project.footingType)
  ...
  const changeFootingType = (footingType: ProjectDocument['footingType']) => { clear(); setProject((current) => ({ ...current, footingType })); setSelectedExampleId(defaultExampleForFootingType(footingType).id); setView('define'); setOpenGroup('loads'); setStatus(footingType === 'strip' ? 'Zapata corrida seleccionada. Las cargas se expresan por metro lineal.' : footingType === 'combined' ? 'Zapata combinada seleccionada. Declara cargas y posición de las dos columnas.' : footingType === 'strap' ? 'Zapata medianera seleccionada. La viga centradora enlaza dos bases sin apoyar en el suelo.' : footingType === 'trapezoidal' ? 'Zapata trapezoidal seleccionada. Los dos anchos extremos definen una base combinada variable.' : footingType === 'edge' ? 'Zapata excéntrica seleccionada. Una cara de la columna coincide con el lindero.' : 'Zapata aislada seleccionada. Las cargas corresponden a la columna centrada.') }
  const loadExample = () => {
    setProject((current) => applyFootingExample(current, selectedExample))
  ...
  const newProject = () => { const fresh = createNewProject(); setProject(fresh); setSelectedExampleId(defaultExampleForFootingType(fresh.footingType).id); setText(inputTextFrom(fresh.inputSnapshot)); setStripText(stripInputTextFrom(fresh.stripInputSnapshot)); setCombinedText(combinedInputTextFrom(fresh.combinedInputSnapshot)); setStrapText(strapInputTextFrom(fresh.strapInputSnapshot)); setTrapezoidalText(trapezoidalInputTextFrom(fresh.trapezoidalInputSnapshot)); setEdgeText(edgeInputTextFrom(fresh.edgeInputSnapshot)); clear(); setMenuOpen(false); setView('define'); setStatus('Proyecto nuevo. Completa los datos o carga un ejemplo didáctico.') }
