...
export default function NewApp() {
  ...
  const loadExample = () => { setProject((current) => ({ ...current, name: 'Zapata rectangular · caso didáctico', inputSnapshot: example })); setText(inputTextFrom(example)); clear(); setOpenGroup('loads'); setStatus('Ejemplo didáctico cargado. No representa un diseño aprobado.') }
  const analyze = () => {
    try {
  ...
  const newProject = () => { const fresh = createNewProject(); setProject(fresh); setText(inputTextFrom(fresh.inputSnapshot)); clear(); setMenuOpen(false); setView('define'); setStatus('Proyecto nuevo. Completa los datos o carga un ejemplo didáctico.') }
