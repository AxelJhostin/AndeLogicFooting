...
export function buildResultCards(analysis: FootingAnalysis | null): ResultCard[] {
  ...
  return [
    ...
    { id: 'flexure-b', title: 'Flexión B', state: flexure ? 'calculated' : 'pending', value: flexure ? `${flexure.widthDirection.momentDemandKnM.toFixed(1)} kN·m` : '—', detail: 'Demanda en cara de columna', destination: 'section' },
    { id: 'flexure-l', title: 'Flexión L', state: flexure ? 'calculated' : 'pending', value: flexure ? `${flexure.lengthDirection.momentDemandKnM.toFixed(1)} kN·m` : '—', detail: 'Demanda en cara de columna', destination: 'section' },
