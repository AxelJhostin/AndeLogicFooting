...
describe('standard profiles', () => {
  it('mantiene un único perfil activo basado en NEC y fuentes públicas', () => {
    ...
    expect(nec.traceability.filter((item) => item.appliesTo.includes('corner')).length).toBeGreaterThan(5)
    expect(nec.traceability.find((item) => item.id === 'corner-punching')?.basis).toBe('Dato externo obligatorio')
    expect(nec.sources.some((source) => source.id === 'fhwa-nhi-06-089')).toBe(true)
    expect(nec.traceability.filter((item) => item.appliesTo.includes('mat'))).toHaveLength(4)
    expect(nec.traceability.find((item) => item.id === 'mat-plate')?.basis).toBe('Dato externo obligatorio')
